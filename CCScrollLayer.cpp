...
//  FirstSample
//
//  Created by Yadong Liu on 9/18/12.
//
//
...
namespace cocos2d
{
	...
	void CCScrollLayer::moveToPage(unsigned int pageNumber)
	{
		...
		CCAction* changePage =
        CCSequence::create(CCMoveTo::create(0.3f, positionForPageWithNumber(pageNumber)),
	...
	{
        CCDirector *pDirector = CCDirector::sharedDirector();
		CCTargetedTouchHandler *handler = (CCTargetedTouchHandler *)pDirector->getTouchDispatcher()->findHandler(this);
		if (handler)
		{
	...
	void CCScrollLayer::cancelAndStoleTouch(CCTouch* pTouch, CCEvent* pEvent)
	{
        ...
        CCDirector *pDirector = CCDirector::sharedDirector();
        pDirector->getTouchDispatcher()->touchesCancelled(touchSet, pEvent);
